...
│   ├── server                                          // 后端服务目录
│   │   ├── shanzhu-admin-exec.jar	                    // 后端打包后的jar文件（需自己添加）
...
│   │   ├── nginx.conf	                                // nginx配置
│   │   ├── shanzhu-doc	                                // 开发文档（https://gitee.com/yukino_git/shanzhu-doc）（需自己添加）
...
### server
> client 为后端部署目录包含：
> shanzhu-admin-exec.jar（后端打包后文件，注意切换application.yml 中 active 为 prod）
> dockerfile（构建镜像）

server构建使用openjdk:21，将shanzhu-admin-exec.jar复制到指定路径下。启动之执行`java -jar` 向外抛出8080端口
...
将项目中`docker`目录上传到服务器，在`docker`目录中执行`docker compose -f name.yaml up -d`时默认会启动`shanzhu-web-server` `shanzhu-web-client` `shanzhu-mysql` `shanzhu-redis` `shanzhu-xxl-job` `shanzhu-web-document`六个容器
